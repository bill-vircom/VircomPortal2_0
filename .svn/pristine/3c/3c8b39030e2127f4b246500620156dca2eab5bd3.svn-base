export const formatStrapiContent = (data: StrapiRawContent) => {
  ...
  data.vpPages.data.forEach(page => {
    const { attributes: { title, uri, description, name, locale, icon, localizations } } = page
    ...
    pages[locale][uri] = { title, description, name, icon }
    ...
    localizations.data.forEach(localizedPage => {
      ...
      pages[lLocale][uri] = { title: lTitle, description: lDescription, name: lName, icon }
